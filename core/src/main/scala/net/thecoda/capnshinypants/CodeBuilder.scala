...
trait CodeBuilderMixin {
  type Universe <: reflect.api.Universe
  def universe: Universe
  
  lazy val stableUniverse = universe
  import stableUniverse._
  ...
  @tailrec final def selectChain(qualifier: Tree, names: String*): Tree = names match {
    case Seq(h, t @ _*) => selectChain(Select(qualifier, TermName(h)), t: _*)
    case _ => qualifier
  }
  
  lazy val pairApply = selectChain(Ident(TermName("scala")), "Tuple2", "apply")
  lazy val mapApply  = selectChain(Ident(TermName("scala")), "Predef", "Map", "apply")
  lazy val seqApply  = selectChain(Ident(TermName("scala")), "Seq", "apply")
...
class CodeBuilder[U <: reflect.api.Universe](val universe: U) extends CodeBuilderMixin {
  type Universe = U
}
...
object CodeBuilder {
  ...
  def forRuntime = new CodeBuilder[ru.type](ru)
  def forMacro[C <: Context](ctx: C) = new CodeBuilder[ctx.universe.type](ctx.universe)
}